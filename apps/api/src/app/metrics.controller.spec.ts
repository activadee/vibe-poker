...
describe('MetricsController', () => {
  ...
  it('returns counters, timing quantiles, and room stats', async () => {
    ...
    // Seed rooms
    rooms.allIds.mockResolvedValue(['A', 'B']);
    rooms.get.mockImplementation(async (id: string) => ({ id, createdAt: Date.now(), expiresAt: Date.now() + 1, participants: id === 'A' ? [{ id: '1', name: 'X', role: 'player' }] : [{ id: '2', name: 'Y', role: 'player' }, { id: '3', name: 'Z', role: 'observer' }] }));

    const json = await controller.get();
    // Counters
